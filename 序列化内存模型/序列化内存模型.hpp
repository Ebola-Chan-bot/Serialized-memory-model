import std;
namespace 序列化内存模型
{
	//你可以把内存句柄本身作为值保存在容器中。重新载入后该句柄仍然有效，但指向的真实内存地址可能已经改变。
	template<typename T>
	using 内存句柄 = T*;
	template<typename T>
	constexpr 内存句柄<T>无效句柄 = static_cast<内存句柄<T>>(-1);
	using size_type = size_t;
	struct 管理器
	{
		const std::function<bool (size_type&,char*&)>分配器;
		/*构造管理器需要用户提供一个底层字节序列特定的分配器。分配器可能被多次调用，应当保证不改变底层字节序列。分配器应当分配不少于输入空间参数值的字节数，并将实际分配的字节数写入空间参数，分配的内存指针写入头指针参数。最后返回一个bool，指示分配的空间是否曾经被管理器写入过：若true，则管理器将可以读取以前保存的内容；否则将覆盖创建新的内容。
		管理器将获得分配器的所有权，管理器删除时分配器也删除。用户应当在分配器的删除过程中负责释放分配的资源。
		*/
		管理器(std::function<bool (size_type& 空间,char*&头指针)>&&分配器) :分配器(std::move(分配器)),文件空间(sizeof(文件头))
		{
			if (!分配器(文件空间, 字节头指针))
				*文件头指针 = { 0,无效值 };
			分配块头指针 = reinterpret_cast<分配块*>(文件头指针 + 1);
		}
		//返回的引用只能临时使用。对管理器的任何操作都可能导致引用失效。
		using T = int;
		//template<typename T>
		T& operator*(内存句柄<T>句柄)
		{
			return *to_address(句柄);
		}
		//返回的指针只能临时使用。对管理器的任何操作都可能导致指针失效。
		//template<typename T>
		T* to_address(内存句柄<T>句柄)
		{
			const size_type 偏移 = sizeof(文件头) + sizeof(分配块) * reinterpret_cast<size_type>(句柄);
			确保至少空间(偏移 + sizeof(分配块));
			const 分配块* 分配块指针 = reinterpret_cast<分配块*>(字节头指针 + 偏移);
			确保至少空间(分配块指针->偏移 + 分配块指针->大小);
			return reinterpret_cast<T*>(字节头指针 + 分配块指针->偏移);
		}
		//返回的句柄可以长期使用，除非deallocate、管理器删除或底层资源被改动。
		//template<typename T>
		内存句柄<T>allocate(size_type 个数 = 1)
		{
			size_type 新块索引;
			size_type 至少空间 = sizeof(文件头);
			for (新块索引 = 0; 新块索引 < 文件头指针->分配块个数; ++新块索引)
			{
				确保至少空间(至少空间 += sizeof(分配块));
				if (分配块头指针[新块索引].偏移 == 无效值)
					break;
			}
			if (新块索引 < 文件头指针->分配块个数) [[likely]]
			{
				分配块头指针[新块索引].偏移 = sizeof(文件头) + sizeof(分配块) * 文件头指针->分配块个数;
				分配块头指针[新块索引].大小 = sizeof(T) * 个数;
				size_type 下块索引 = 文件头指针->第一块;
				//由于新块索引小于已有块个数，所以第一块必定有效。
				确保至少空间(sizeof(文件头) + sizeof(分配块) * (下块索引 + 1));
				分配块* 下块指针 = 分配块头指针 + 下块索引;
				size_type 上块索引 = 无效值;
				分配块* 新块指针 = 分配块头指针 + 新块索引;
				while (下块指针->偏移 < 新块指针->偏移 + 新块指针->大小)
				{
					新块指针->偏移 = 下块指针->偏移 + 下块指针->大小;
					上块索引 = 下块索引;
					下块索引 = 下块指针->下一块;
					if (下块索引 == 无效值)
					{
						新块指针->下一块 = 无效值;
						return reinterpret_cast<内存句柄<T>>(下块指针->下一块 = 新块索引);
					}
					确保至少空间(sizeof(文件头) + sizeof(分配块) * (下块索引 + 1));
					下块指针 = 分配块头指针 + 下块索引;
					新块指针 = 分配块头指针 + 新块索引;
				}
				新块指针->下一块 = 下块索引;
				if (上块索引 == 无效值)
					文件头指针->第一块 = 新块索引;
				else
					分配块头指针[上块索引].下一块 = 新块索引;
			}
			else
			{
				//块空间不足，需要2倍扩张
				文件头指针->分配块个数 = (新块索引 + 1) * 2;
				//原有数据块现在可能占用了新的分配块位置，需要向后挪
				size_type 下块索引 = 文件头指针->第一块;
				size_type 数据块偏移 = sizeof(文件头) + sizeof(分配块) * 文件头指针->分配块个数;
				std::queue<char>缓冲区;
				size_type 上块索引 = 无效值;
				while (下块索引 != 无效值)
				{
					//旧块全部扫描过了，再次扫描不需要检查空间分配
					if (分配块头指针[下块索引].偏移 >= 数据块偏移)[[unlikely]]
					{
						//写出头后有空余空间可写，先尝试清缓存
						size_type 字节数 = std::min(分配块头指针[下块索引].偏移 - 数据块偏移, 缓冲区.size());
						确保至少空间(数据块偏移 + 字节数);
						char* 写出头 = 字节头指针 + 数据块偏移;
						数据块偏移 += 字节数;
						for ( ; 字节数; --字节数)
						{
							*写出头++ = 缓冲区.front();
							缓冲区.pop();
						}
						if (缓冲区.empty())
							//如果缓冲区已清空，说明后续数据块无需再挪动
							break;
					}
					//当前块数据无法直接写出，需要缓存。上个if块中可能发生过重分配，必须重新取得块指针。
					确保至少空间(分配块头指针[下块索引].偏移 + 分配块头指针[下块索引].大小);
					分配块& 下块引用 = 分配块头指针[下块索引];
					下块引用.偏移 = 数据块偏移 + 缓冲区.size();
					缓冲区.push_range(std::span<const char>(字节头指针 + 下块引用.偏移, 下块引用.大小));
					上块索引 = 下块索引;//保存上块索引，分配新块时将要用到
					下块索引 = 下块引用.下一块;
				}
				//将缓冲区剩余数据写出
				if (缓冲区.size())
				{
					确保至少空间(数据块偏移 + 缓冲区.size());
					char* 写出头 = 字节头指针 + 数据块偏移;
					数据块偏移 += 缓冲区.size();
					while (缓冲区.size())
					{
						*写出头++ = 缓冲区.front();
						缓冲区.pop();
					}
				}
				//分配新块
				确保至少空间(sizeof(文件头) + sizeof(分配块) * 文件头指针->分配块个数);
				分配块& 新块引用 = 分配块头指针[新块索引];
				新块引用.大小 = sizeof(T) * 个数;
				if (上块索引 == 无效值)
				{
					文件头指针->第一块 = 新块索引;
					新块引用.下一块 = 下块索引;
				}
				else
				{

				}
				新块引用.偏移 = 数据块偏移;
			}
		}
		//释放后的句柄不再可用。
		template<typename T>
		void deallocate(内存句柄<T>句柄);
		//重分配不改变句柄本身的值，但可能会改变句柄指向的真实内存地址。
		template<typename T>
		void reallocate(内存句柄<T>句柄, size_type 个数);
	protected:
		static constexpr size_type 无效值 = -1;
		struct 分配块
		{
			size_type 偏移;
			size_type 大小;
			size_type 下一块;
		};
		struct 文件头
		{
			size_type 分配块个数;//空块必须用无效值填充偏移字段
			size_type 第一块;
		};
		union
		{
			char* 字节头指针;
			文件头* 文件头指针;
		};
		分配块* 分配块头指针;
		size_type 文件空间;
		void 确保至少空间(size_type 至少空间)
		{
			if (至少空间 > 文件空间)[[unlikely]]
			{
				文件空间 = 至少空间 * 2;
				分配器(文件空间, 字节头指针);
				分配块头指针 = reinterpret_cast<分配块*>(文件头指针 + 1);
			}
		}
	};
}