export module 序列化内存模型;
import std;
export namespace 序列化内存模型
{
	//你可以把内存句柄本身作为值保存在容器中。重新载入后该句柄仍然有效，但指向的真实内存地址可能已经改变。内存句柄本身就是指针别名，但不是真正意义上的指针，不能进行C++指针运算。
	template<typename T>
	using 内存句柄 = T*;
		...
		void reallocate(内存句柄<T>句柄, size_type 个数)
		{
			...
			if (块值.下一块 != 无效值)
			{
				...
				if (分配块头指针[块值.下一块].偏移 < 块值.偏移 + 块值.大小)
				{
					...
					块值.下一块 = 文件头指针->第一块;
					//已知空间不足，所以第一块不可能无效
					块值.偏移 = sizeof(文件头) + sizeof(分配块) * 文件头指针->分配块个数;
					分配块* 下块指针;
					do
					{
						确保至少空间(sizeof(文件头) + sizeof(分配块) * (块值.下一块 + 1));
						下块指针 = 分配块头指针 + 块值.下一块;
						if (块值.偏移 + 块值.大小 <= 下块指针->偏移)
							break;
						块值.偏移 = 下块指针->偏移 + 下块指针->大小;
						块值.上一块 = 块值.下一块;
						块值.下一块 = 下块指针->下一块;
					} while (块值.下一块 != 无效值);
					if (块值.上一块 == 无效值)
